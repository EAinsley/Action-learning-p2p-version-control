...
#align(center)[
  #v(2cm)
  #text(size: 20pt, weight: "bold")[Peer-to-Peer Directory Synchronization] \
  #v(2mm)
  #text(size: 16pt, weight: "bold")[with Lightweight Version Control] \
  #v(1cm)
  #text(size: 14pt, weight: "medium")[Thesis Pending Parts (Completed)] \
  #v(2cm)
  #text(size: 12pt)[
    *Authors:* \
    Arihant Jain \
...
- The Go coordinator `coordinator.go` was refactored into a Multi-Repo Coordinator, managing isolated repository configurations.
- During file transfers, the `repo_id` is passed within the `prepare_file_transfer` payload.
...
- To prevent translocation and permission failures, the Unix domain socket and database paths are resolved dynamically by `IpcBridge.java` and passed to the Go daemon via environment variables (`IPC_SOCKET` and `DB_PATH`).
...
- *Supervisor Restart Pattern:* The Java frontend `IpcBridge.java` was updated with a reconnection loop. If it encounters three consecutive socket failures, it assumes the Go coordinator has terminated, kills any stale processes, and automatically spawns a fresh backend instance.
- *Listener Leak Prevention:* During UI view transitions (such as closing repository tabs or windows), controller instances were retaining active listeners in `IpcBridge`. This caused memory leaks and duplicate UI updates. We introduced a `removeListener()` registration mechanism in `IpcBridge.java` and wired it to `WINDOW_CLOSE_REQUEST` handlers in the controllers (such as `RepoStatusController.java`) to cleanly unregister all IPC listeners.
...
- A Python integration script, `integration_harness.py`, was created to simulate a two-peer network environment on a single loopback interface. The harness cleans temporary folders, compiles binaries, launches isolated coordinator and watcher instances, modifies file states, and programmatically asserts file and permission replication.
- In parallel, Go integration tests in `integration_test.go` were created to programmatically assert logical clock tick/witness mechanics, concurrent edit detection, metadata conflicts, and LWW push-back logic, bypassing network noise for algorithm verification.
...
- An "Add Peer" option was implemented in the JavaFX frontend `RepoStatusController.java`, allowing users to input a target peer's IP address and coordination port.
- This inputs a manual payload to the Go connection manager `connection_manager.go`, bypassing mDNS discovery and establishing a direct TCP connection.
...
- *Java Frontend:* Implemented repository dashboard, peer list, manual connection dialog, version history table, and a theme toggle system (Light/Dark themes styled using dynamic CSS variables in `styles.css`, `dark.css`, and `light.css`).
